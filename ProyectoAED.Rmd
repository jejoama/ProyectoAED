...
abstract: |
  ...
  Los resultados clave validados mediante pruebas de t de student, demuestran que la disparidad no reside en la tasa de paro sino en la tasa de actividad. 
...
# Introducción
Este artículo presenta un enfoque en R para el Análisis Exploratorio de Datos de la Encuesta de Población Activa (EPA), centrándose en la evolución histórica de la brecha de género en el mercado laboral español. Utilizando el ecosistema `tidyverse`, el estudio comienza con una limpieza y transformación de los datos empleando `dplyr` y el cálculo de Tasa de paro, actividad y ocupación, claves para el análisis. Seguido de la visualización con `ggplot2` para ilustrar la evolución temporal. Por último, se validan los resultados con pruebas de *t Student* para confirmar la significancia estadística de los resultados.
...
Para este análisis nos hemos basado principalmente en las tasas de paro y de actividad por considerarlas los indicadores más representativos del merrcado laboral. Son los mejores indicadores porque permite entender tanto quién participa en el mercado laboral mediante la tasa de actividad como quién logra empleo dentro de él con la tasa de paro.



Los datos se han obtenido en el INE a fecha de 21/10/2025.
...
Los valores NA se encuentran concentrados en la variable `Total`, mostrando una ausencia específica en las categorías de situación laboral relacionadas con el desempleo com ose muestra en la siguiente figura:
...
- **Imputación de NA a 0**: Los valores faltantes se concentran en categorías como Personas mayores en paro o que buscan su primer empleo. El INE suprime el dato por ser un valor insignificante, por lo que se opta por remplazar los valores nulos a 0 para no perder el resto de información válida de la instancia.

- **Creación de tasas**: Se añaden las columnas `Tasa_paro`, `Tasa_actividad` y `Tasa_ocupacion` utilizando `mutate()` y las expresiones de la EPA (3), (4) y (5).
...
Para identificar outliers se va a utilizar el método del rango intercuartílico (IQR)

```{r}

# 1. Función para detectar límites del IQR
detectar_outliers_iqr <- function(df, tasa_col) {
  # Calcular Q1, Q3 y IQR globalmente para la tasa
  Q1 <- quantile(df[[tasa_col]], 0.25, na.rm = TRUE)
  Q3 <- quantile(df[[tasa_col]], 0.75, na.rm = TRUE)
  IQR_val <- Q3 - Q1
  
  limite_superior <- Q3 + 1.5 * IQR_val
  limite_inferior <- Q1 - 1.5 * IQR_val
  
  # Añadir columna booleana que indica si es outlier
  df %>%
    mutate(
      !!paste0("Outlier_", sub("Tasa_", "", tasa_col)) := case_when(
        .data[[tasa_col]] > limite_superior ~ "Atípico Superior",
        .data[[tasa_col]] < limite_inferior ~ "Atípico Inferior",
        TRUE ~ "Normal"
      )
    )
}

# 2. Aplicar y buscar los outliers en las dos tasas
df_outliers <- df_clean %>%
  detectar_outliers_iqr("Tasa_actividad") %>%
  detectar_outliers_iqr("Tasa_paro")

# 3. Filtrar y mostrar los registros atípicos en Tasa de Actividad
outliers_actividad <- df_outliers %>%
  filter(Outlier_actividad != "Normal") %>%
  select(Periodo, Sexo, Edad, Tasa_actividad, Outlier_actividad)

# Para visualizar los resultados en el informe (opcional)
print("Outliers en Tasa de Actividad:")
#knitr::kable(outliers_actividad)

# 4. Los outliers se mostrarán automáticamente en tus box plots (puntos fuera de los bigotes)
```
```{r}
...
reglaboxplot<-function(x){
  ...
  xu<-quantile(x, 0.75, na.rm = TRUE)
  xi<-quantile(x, 0.25, na.rm = TRUE)
  IQR<-xu-xi
  outliers<-if_else((x>xu+1.5*IQR)|(x<xi-1.5*IQR),T,F)
  return(outliers)
}

# Ejemplo general
lapply(split(df_clean, df_clean$grupo_edad), function(x) {
  boxplot(x$variable_interes,
          main = paste("Grupo de edad:", unique(x$grupo_edad)),
          ylab = "Variable de interés")
})
...
```{r,echo = FALSE}
ggplot(df_clean, aes(x = Sexo, y = Tasa_paro, fill = Sexo)) +
  geom_boxplot() +
  labs(
    title = "Distribución de la Tasa de Actividad por Sexo (Análisis Global)",
    subtitle = "El boxplot visualiza la mediana, la dispersión y la asimetría de la brecha.",
...
test_paro <- t.test(Tasa_paro ~ Sexo, data = df_clean)
...
Por otro lado, la tasa de paro muestra un valor de p alto ($0.128$) lo que sugiere que la tasa de paro no depende del sexo. 



# Conclusión

Mediante el análisis gráfico se ha observado que la tasa de actividad masculina es siempre superior, aunque se va reduciendo progresivamente. También se aprecia que las mayores diferencias se dan en los grupos de mediana edad que coinciden con las etapas de mayor carga familiar y laboral.
...
Para completar las gráficas de una forma rigurosa se ha hecho una prueba de t de student en la que se ha obtenido como resultado que la dependencia entre la tasa de actividad y el sexo es estadísticamente significativo mientras que la tasa de paro y el sexo no lo es.
Esto nos permite concluir que la brecha de género existe en el mercado laboral, no se debe a la dificultad de acceso al mercado laboral por parte de las mujeres sino que se explica más por la menor participación activa de las mujeres que podría venir de dinámicas de inclusión laboral ineficientes.

Por tanto, con este análisis sobre la brecha de genero hemos podido detectar como esta brecha no se encuentra realmente en el desempleo, como es de creencia popular, sino en la participación activa. Pese a que las mujeres se han ido incrementando progresivamente en el mercado económico laboral, aun sufren muchas barreras a las que deben hacer frente. Esta dificultad es más clara en edad mas centrales. 
