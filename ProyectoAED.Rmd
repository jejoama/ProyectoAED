...
Para identificar outliers se va a utilizar el método del rango intercuartílico (IQR). Se obtiene que para la tasa de actividad no hay valores outliers, sin embargo, para la tasa de paro sí. Se tendrá en cuenta 
...
df_clean %>% 
  ...
  guides(fill = "none", # Quita la leyenda 'Sexo' generada por aes(fill=Sexo)
         color = guide_legend(title = "Outliers")) + # Da un título a la leyenda de outliers
...
A diferencia de la tasa de actividad dodne no se han detectado outliers, la tasa de paro muestra una alta concentración de valores atípicos en ambos sexos. Los puntos atípicos se han filtrado y coloreado según el rango de edad.

Los valores atípicos en la tasa de paro están ligados a la variable de edad. Estos valores no son errores de registro sino la muestra del paro juvenil.
