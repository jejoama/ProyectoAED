...
detectar_outliers_iqr <- function(df, tasa) {
  Q1 <- quantile(df[[tasa]], 0.25, na.rm = TRUE)
  Q3 <- quantile(df[[tasa]], 0.75, na.rm = TRUE)
  IQR_val <- Q3 - Q1
  ...
  df %>%
    mutate(
      !!paste0("Outlier_", sub("Tasa_", "", tasa)) := case_when(
        .data[[tasa]] > limite_superior ~ "Atípico Superior",
        .data[[tasa]] < limite_inferior ~ "Atípico Inferior",
        TRUE ~ "Normal"
      )
    )
}
...
```{r,echo = FALSE}
df_outliers %>% ggplot(aes(x = Sexo, y = Tasa_paro)) +
